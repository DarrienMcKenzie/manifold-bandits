...
# DM:
# The format of the hyperparameters are a little different than the other experiment files
# I assume you just want to build a Latent Task Tree, and do not want to run BMC
# Thus, this file will be split into TREE_RELEVANT and NOT RELEVANT sections
# You don't *have* to use this file to build a tree; it's just a convenience

##### TREE RELEVANT #####
TREEVIZ_ONLY=True # if true, it won't try to run BMC after tree construction

# Data
TRAIN_FILE='' # put file path containing prompts here
PROMPT_KEY='prompt' # key/field of dataframe/file that contains the prompts; needs to be in a nested format, like so:  [{"content": "What is 2+2?","role": "user"}]

# Model
MODEL_PATH='' # path to HF or local model; example='Qwen/Qwen3-8B-Base'
# Note: we haven't tested *every* LLM / VLM / VLA under the sun, so you *may* have to make code adjustments depending on model choice
# Newer models like Gemma-4 and Qwen3.5 require some manual code adjustments + version updates for instance; I haven't gotten around to fixing it at the time of public release
# feel free to also open an Issue / Pull Request if something breaks

# Analysis settings
ANALYZE=True # save word clouds per node, example problems per node, node percentages, etc
ANALYSIS_FILE='analysis' # where analysis related files are stored (word clouds, prompts, etc)
PROMPTS_PER_NODE=10 # amount of prompts per node/cluster to store when analyzing tree

# Compute
NNODES=1 # has NOT been tested for multiple nodes
NGPUS_PER_NODE=8
NCPU_CORES=32

## Latent loading
TRAIN_BATCH_SIZE=128
LATENT_BATCH_FACTOR=1 # multiple of batch size used to load latents (1 is very conservative, could go higher, depends on machine and datset size)


# Below are the hyperparameters used in the paper. You're free to change these; but if you just want a tree, I wouldn't mess with them
...
# BMC-T
TARGETING=False
TARGETING_GAMMA=0.25 # useful for demonstration
TARGETING_RHO=0.05
...
python -m main_bmc \
    ...
    +algorithm.bmc.treeviz_only=${TREEVIZ_ONLY} \
    +algorithm.bmc.analyze=${ANALYZE} \
    +algorithm.bmc.analysis_file=${ANALYSIS_FILE} \
    +algorithm.bmc.prompts_per_node=${PROMPTS_PER_NODE} \
