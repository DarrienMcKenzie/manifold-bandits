...
## Experiment
PROJECT_NAME='mma'
EXP_NAME='MB_GRPO-BMC_DAPO-Math_Qwen3-4B-Base'
EPOCHS=8 
CKPTS_DIR='volume'
...
CLIP_RATIO_HIGH=0.2 #from gspo_trainer/run_qwen30b_gspo.sh
CLIP_RATIO_LOW=0.2 #from gspo_trainer/run_qwen30b_gspo.sh


## GSPO/DAPO feature: No KL
USE_KL_LOSS=True
USE_KL_IN_REWARD=False 
KL_COEF=0.001 # for GRPO
KL_LOSS_COEF=0.001
...
TREEVIZ_ONLY=False # for tree visualization only (not a BMC RL run)
...
# A reference for the Normal-Inverse-Gamma (NIG) distribution: https://en.wikipedia.org/wiki/Normal-inverse-gamma_distribution
LAMBDA_MIN=0.2
LAMBDA_MAX=10.0
SIGMA2_MIN=0.05
...
python -m main_bmc \
    ...
    +algorithm.bmc.ltt.enable=${USE_LTT} \
