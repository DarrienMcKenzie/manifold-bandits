...
class RayDAPOTrainer(RayPPOTrainer):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # MODIFIED:
                    # we keep track of these metrics even if we're doing uniform sampling
                    # these are for logging in both cases
                    ...
                    avg_prompt_var = np.mean(np.asarray(batch_var)) #ORIGINAL
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        batch = new_batch
                        metrics["train/zero_variance_ratio"] = 1-(len(kept_prompt_uids) / self.config.data.train_batch_size) #MB: tracking non-zero ratio
                    else:
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            ## Duplicating metric tracking code for dynamic sampling case (evaluate variance of combined batches)
                            ##
                            # Collect the sequence reward for each trajectory
                            prompt_uid2metric_vals = defaultdict(list)
                            for uid, metric_val in zip(
                            ...
                            for prompt_uid, metric_vals in prompt_uid2metric_vals.items():
                                prompt_uid2metric_std[prompt_uid] = np.std(metric_vals)
                                batch_var.append(np.var(metric_vals))
                            avg_prompt_var = np.mean(np.asarray(batch_var)) #override
                    ...
                    with marked_timer("testing", timing_raw, "green"):
                        if self.global_steps < self.config.trainer.dense_cutoff:
                            ...
                            if self.global_steps % (self.config.trainer.test_freq * 2) == self.config.trainer.test_freq:  
                                ...
                                val_metrics: dict = self.validate_train()  
                            else:  
                            ...
                            train_val_metrics: dict = self.validate_train() 
    ...
    def validate_train(self):
        """Evaluate pass@1 on entire training set"""  
